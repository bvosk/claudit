...
class CaptureAddon:
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        self.logger.info(f"Intercepted request #{self.request_count}: {flow.request.method} {flow.request.pretty_url}")
    ...
    def response(self, flow: http.HTTPFlow) -> None:
        ...
        try:
            ...
            capture_data = {
                ...
                "timestamp": datetime.fromtimestamp(flow.response.timestamp_start).isoformat(),
                "request": request_info,
                "response": response_info,
                "duration_ms": round((flow.response.timestamp_start - flow.request.timestamp_start) * 1000, 2)
            }
            
            # Write to file as JSON lines format
            self._write_capture_data(capture_data)
            
            self.logger.info(f"Captured response: {flow.response.status_code} for {flow.request.pretty_url}")
